...
function MainSection2({selectedAlgorithm}) {
  ...
  const transformFormData = (formData) => {
    ...
    transformedData.name = selectedAlgorithm[0].algorithm_name;
  ...
  if (selectedAlgorithm !== null)
    return (
      <div className="bg-gray-100 p-4 rounded h-full">
        <h2 className="text-2xl font-bold mb-8 pb-0">{selectedAlgorithm[0].algorithm_name}</h2>
        <div className="grid grid-cols-12 p-3 bg-white rounded-xl">
          ...
          <div className="col-span-6 m-4 p-4 border-2 border-gray-300 rounded-lg">
            ...
            <FormGenerator algorithmData={selectedAlgorithm[0]} formData={formData} handleChange={handleChange}/>
          ...
          <div className="col-span-6 m-4 p-4 border-2 border-gray-300 rounded-lg">
            ...
            {selectedAlgorithm[0].algorithm_output && Object.entries(output).map(([k, v], idx) => (
              <div key={idx} className="flex mb-2.5">
                <label className="w-1/2 block my-2">{k || ''}</label>
                <input
                  type="text"
                  value={v || ''}
